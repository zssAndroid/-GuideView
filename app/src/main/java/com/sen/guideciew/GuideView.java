...
public class GuideView extends RelativeLayout {
    ...
    private void drawMaskLayer(Canvas canvas) {
        ...
        if (!"0".equals(mTranslateX)) {
            mTargetViewLocationX = mTargetViewLocation[0] + mTranslateX;
        } else {
            mTargetViewLocationX = mTargetViewLocation[0];
        }

        RectF rectF = new RectF(mTargetViewLocationX - mTransparentPaddingLeft + mTransparentMarginLeft,
                mTargetViewLocation[1] - mTransparentPaddingTop + mTransparentMarginTop,
                mTargetViewLocationX + mTargetViewWidth + mTransparentPaddingRight - mTransparentMarginRight,
                mTargetViewLocation[1] + mTargetViewHeight + mTransparentPaddingBottom - mTransparentMarginBottom);

        switch (mFormType) {
            case Form.CIRCLE:
                mTemp.drawCircle(mTargetViewLocationX + mTargetViewWidth / 2, mTargetViewLocation[1] + mTargetViewWidth / 2, mTargetViewWidth / 2, mTransparentPaint);
                break;
            case Form.ELLIPSE:
                mTemp.drawOval(rectF, mTransparentPaint);
                break;
            case Form.REACTANGLE:
                mTemp.drawRect(mTargetViewLocationX, mTargetViewLocation[1], mTargetViewLocationX + mTargetViewWidth, mTargetViewLocation[1] + mTargetViewHeight, mTransparentPaint);
                break;
            case Form.CIRCLE_LONG:
                int longRadius = Math.max(mTargetViewWidth, mTargetViewHeight);
                mTemp.drawCircle(mTargetViewLocation[0] + mTargetViewWidth / 2, mTargetViewLocation[1] + mTargetViewWidth / 2, longRadius / 2, mTransparentPaint);
                break;
            case Form.CIRCLE_SHORT:
                int shortRadius = Math.min(mTargetViewWidth, mTargetViewHeight);
                mTemp.drawCircle(mTargetViewLocation[0] + mTargetViewWidth / 2, mTargetViewLocation[1] + mTargetViewWidth / 2, shortRadius / 2, mTransparentPaint);
                break;
        }
        //绘制到GuideView的画布上
    ...
    public static class Builder {
        ...
        public Builder setmForm(int mForm) {
            mParams.mForm = mForm;
            return this;
        }
